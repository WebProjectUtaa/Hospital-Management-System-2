...
async def login(request):
    """
    User login endpoint with Appointment Service and Notification Service integration.
    """
    data = request.json
    ...
    if not email or not password:
        return sanic_response.json({"error": "Email and password are required"}, status=400)
    ...
    async with await get_db_connection() as conn:
        try:
            ...
            if user:
                ...
                if role == "patient":
                    try:
                        ...
                        if appointment_response.status_code == 200:
                            ...
                            print(f"Failed to fetch appointments: {appointment_response.status_code}")
                ...
                return sanic_response.json({"error": "Invalid credentials"}, status=401)
        except Exception as e:
            return sanic_response.json({"error": str(e)}, status=500)
...
async def refresh_token(request):
    ...
    if not refresh_token:
        return sanic_response.json({"error": "Refresh token is missing"}, status=400)
    ...
    try:
        ...
        notification_data = {
            "to_email": decoded_token.get("email", "unknown@example.com"),
        ...
        return sanic_response.json({"access_token": access_token}, status=200)
    except jwt.ExpiredSignatureError:
        return sanic_response.json({"error": "Refresh token expired"}, status=401)
    except jwt.InvalidTokenError:
        return sanic_response.json({"error": "Invalid token"}, status=401)
    except Exception as e:
        return sanic_response.json({"error": str(e)}, status=500)
...
async def logout_user(request):
    ...
    if not auth_header or not auth_header.startswith("Bearer "):
        return sanic_response.json({"error": "Unauthorized"}, status=401)
    ...
    return sanic_response.json({"message": "Logout successful. Redirect to login page."}, status=200)
...
async def validate_token(request):
    ...
    if not token:
        return sanic_response.json({"error": "Token is missing"}, status=400)

    try:
        decoded = jwt.decode(token, SECRET_KEY, algorithms=["HS256"])
        return sanic_response.json({"message": "Token is valid", "data": decoded}, status=200)
    except jwt.ExpiredSignatureError:
        return sanic_response.json({"error": "Token has expired"}, status=401)
    except jwt.InvalidTokenError:
        return sanic_response.json({"error": "Invalid token"}, status=401)
